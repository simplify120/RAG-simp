"""
build_embedding_index_test_set.py (E5 version)
----------------------------------------------
Backfills public.dataset_item_embeddings_e5_768_test_set with E5 embeddings
for the TEST split of dataset_items (the 40 items selected with random.seed(42)).

Usage:
    python -m app.experiments.RAG.e5.build_embedding_index_test_set          # skip already-embedded items
    python -m app.experiments.RAG.e5.build_embedding_index_test_set --force  # recompute all embeddings
"""
...
def upsert_embeddings(
    ...
    """
    Upserts a batch of embeddings into public.dataset_item_embeddings_e5_768_test_set.
    """
    sql = text("""
        INSERT INTO public.dataset_item_embeddings_e5_768_test_set (item_id, text_adv, embedding)
...
def _already_embedded_ids(db) -> set:
    rows = db.execute(
        text("SELECT item_id FROM public.dataset_item_embeddings_e5_768_test_set")
    ).fetchall()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Backfill public.dataset_item_embeddings_e5_768_test_set with E5 embeddings."
    )
    ...
    try:
        ...
        row_count = db.execute(
            text("SELECT COUNT(*) FROM public.dataset_item_embeddings_e5_768_test_set")
        ).scalar()
