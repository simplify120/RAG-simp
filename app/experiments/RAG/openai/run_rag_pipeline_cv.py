"""
RAG pipeline (CV): evaluate one complement fold at a time (37 or 38 items).

Retrieval corpus = other 3 folds of the complement + all 40 test items (see rag_cv_v1.json).
...
Usage (from repo root):
    ...
    # All folds 0–3 in one process (sequential):
    python -m app.experiments.RAG.openai.run_rag_pipeline_cv \\
        --model sonar --description "step 2 - RAG top k=3" --top-k 3 --all-folds

    # Default is 12s between items. For very strict RPM caps, raise further:
    python -m app.experiments.RAG.openai.run_rag_pipeline_cv \\
        --model gemini --description "step 2 - RAG top k=3" --top-k 3 --all-folds --sleep-seconds 20
"""
...
from app.experiments.RAG.openai.retrieval import retrieve_top_k_cv
...
async def run_pipeline(
    ...
    try:
        ...
        print(
            f"RAG CV pipeline (OpenAI embeddings): model={model}, "
            f"description={description_out}, top_k={top_k}, fold={fold}"
        )
        ...
        for idx, (item_id, text_adv) in enumerate(query_items, 1):
            ...
            if not retrieved:
                print(
                    f"  Warning: No retrieved examples for item {item_id}, proceeding without RAG context"
                )
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            print(
                                f"  Error: {str(e)[:80]}. Retry {attempt + 1}/{max_retries} in {wait:.0f}s..."
                            )
                            await asyncio.sleep(wait)
                            continue
                        raise
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run RAG pipeline (CV fold) for text simplification (OpenAI embeddings).",
    )
    parser.add_argument("--model", required=True, choices=list(MODEL_CONFIG.keys()))
    parser.add_argument("--description", required=True, help="Description for stored results (fold tag appended)")
    ...
    fold_group.add_argument(
        ...
        type=int,
        choices=[0, 1, 2, 3],
        help="Complement fold index (0–3) to use as queries",
    )
    fold_group.add_argument(
        "--all-folds",
        action="store_true",
        help="Run folds 0, 1, 2, and 3 sequentially (same as four separate --fold runs).",
    )
    parser.add_argument(
        "--strategy",
        default="all",
    ...
    parser.add_argument("--limit", type=int, default=None, help="Limit number of query items (for testing)")
    parser.add_argument(
        "--sleep-seconds",
        type=float,
        default=CV_DEFAULT_SLEEP_SECONDS,
        metavar="SEC",
        help="Seconds to wait before each new query item after the first (default: 12). "
        "Also spaces strategies within an item (max(4s, half of this value)).",
    )
